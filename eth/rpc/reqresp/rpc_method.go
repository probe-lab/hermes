...
type (
	ReadRequestFn       func(dest interface{}) error
	WriteSuccessChunkFn func(data interface{}) error
	WriteMsgFn          func(msg string) error
)
...
func (m *RPCMethod) MakeStreamHandler(ctx context.Context, comp Compression, listener OnRequestListener, timeout time.Duration) network.StreamHandler {
	...
	}).MakeStreamHandler(ctx, comp, m.RequestCodec.MinByteLen(), m.RequestCodec.MaxByteLen(), timeout)
}
